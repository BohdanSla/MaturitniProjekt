...
if(isset($_SESSION["user"])) {
    ...
    if(count($produkty) > 0) {
        ...
        if(count($arr) > 0) {
            ...
            $objednavka .= "<tr><td colspan='4'><b style='display:flex;justify-content:flex-end;'>Slevový kód: -". $arr[0]["sleva"] ." Kč</b></td></tr>";
        }
    ...
    if(isset($_POST["uplatnit"])) {
        ...
        $stmt = $db->prepare("SELECT id_slevoveho_kodu,id_sportu AS kod FROM slevovy_kod_sport JOIN slevovy_kod ON slevovy_kod.id = slevovy_kod_sport.id_slevoveho_kodu WHERE id_sportu IN ( SELECT id_sportu FROM produkt WHERE nazev IN (:nazev)) AND kod = :kod AND expirace > CURRENT_DATE() AND bylPouzit = 0 UNION SELECT id_slevoveho_kodu,id_znacky FROM slevovy_kod_znacka JOIN slevovy_kod ON slevovy_kod.id = slevovy_kod_znacka.id_slevoveho_kodu WHERE id_znacky IN ( SELECT id_znacky FROM produkt WHERE nazev IN (:nazev)) AND kod = :kod AND expirace > CURRENT_DATE() AND bylPouzit = 0;");
        
        $stmt->execute([":nazev" => $nazvyProduktu,":kod" => $_POST["kod"]]);
